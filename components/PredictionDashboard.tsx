    ...
    <div className="w-full max-w-7xl mx-auto space-y-10 animate-in fade-in slide-in-from-bottom-8 duration-1000">
      <header className="text-center space-y-4">
        <div className="inline-flex items-center px-4 py-1.5 bg-blue-600/10 border border-blue-600/20 rounded-full text-blue-400 text-[10px] font-black uppercase tracking-[0.3em]">
          {data.sport} Tactical Grounding Active
        </div>
        <div className="flex items-center justify-center gap-6 md:gap-16 px-4">
          ...
          <div className="flex flex-col items-center flex-shrink-0">
            <div className="w-10 h-10 md:w-16 md:h-16 rounded-full border border-slate-700 flex items-center justify-center bg-slate-900 shadow-inner">
              <span className="text-slate-500 font-black text-lg italic select-none">VS</span>
            </div>
          </div>
      ...
      {data.sources && data.sources.length > 0 && (
        <div className="glass-card p-6 rounded-3xl border-white/5 mx-4">
          <h4 className="text-white font-black text-[10px] uppercase tracking-widest mb-4 flex items-center gap-2 text-slate-400">
          ...
          <div className="flex flex-wrap gap-2">
            {data.sources.map((source, i) => (
              <a 
                ...
                className="bg-slate-800/30 hover:bg-slate-700/50 border border-slate-700/50 px-3 py-1.5 rounded-xl text-[10px] text-slate-400 transition-colors flex items-center gap-2"
              >
      ...
      <div className="glass-card p-6 rounded-3xl border-white/5 flex flex-col md:flex-row items-center justify-between gap-6 mx-4">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h4 className="text-white font-black text-sm uppercase tracking-tight">AI Multi-Sport Verification</h4>
            <p className="text-slate-500 text-xs">Analysis cross-referenced with recent {data.sport} results and tactical data.</p>
          </div>
        </div>
        <div className="text-[9px] text-slate-500 font-black uppercase tracking-[0.2em] bg-slate-950 px-4 py-2 rounded-xl border border-slate-800">
          Powered by Gemini 3 Pro • Web Search Tool
        </div>
      </div>
    </div>
  );
